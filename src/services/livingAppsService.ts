...
import type { Reise, Kategorie, Budgetposten, Ausgabe, CreateReise, CreateKategorie, CreateBudgetposten, CreateAusgabe } from '@/types/app';
...
export class LivingAppsService {
  // --- REISE ---
  static async getReise(): Promise<Reise[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REISE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Reise[];
    return enrichLookupFields(records, 'reise');
  }
  static async getReiseEntry(id: string): Promise<Reise | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.REISE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Reise;
    return enrichLookupFields([record], 'reise')[0];
  }
  static async createReiseEntry(fields: CreateReise) {
    return callApi('POST', `/apps/${APP_IDS.REISE}/records`, { fields: cleanFieldsForApi(fields as any, 'reise') });
  }
  static async updateReiseEntry(id: string, fields: Partial<CreateReise>) {
    return callApi('PATCH', `/apps/${APP_IDS.REISE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reise') });
  }
  static async deleteReiseEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.REISE}/records/${id}`);
  }
  ...
  static async createKategorieEntry(fields: CreateKategorie) {
    return callApi('POST', `/apps/${APP_IDS.KATEGORIE}/records`, { fields: cleanFieldsForApi(fields as any, 'kategorie') });
  }
  static async updateKategorieEntry(id: string, fields: Partial<CreateKategorie>) {
    return callApi('PATCH', `/apps/${APP_IDS.KATEGORIE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kategorie') });
  }
  ...
  // --- AUSGABE ---
  static async getAusgabe(): Promise<Ausgabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ausgabe[];
    return enrichLookupFields(records, 'ausgabe');
  }
  static async getAusgabeEntry(id: string): Promise<Ausgabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSGABE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ausgabe;
    return enrichLookupFields([record], 'ausgabe')[0];
  }
  static async createAusgabeEntry(fields: CreateAusgabe) {
    return callApi('POST', `/apps/${APP_IDS.AUSGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'ausgabe') });
  }
  static async updateAusgabeEntry(id: string, fields: Partial<CreateAusgabe>) {
    return callApi('PATCH', `/apps/${APP_IDS.AUSGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausgabe') });
  }
  static async deleteAusgabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AUSGABE}/records/${id}`);
  }
